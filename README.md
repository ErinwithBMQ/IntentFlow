...
“项目设置”可调整显示名称、项目 Prompt、`test`/`build` 受控命令参数和忽略目录。项目 Prompt
会作为该项目下每次 AI 回复、规划和 Agent 执行的固定上下文；命令参数每行填写一个，
`{workspace}` 表示当前项目目录，配置不会经过 Shell 执行。

顶部“模型配置”可在页面内保存唯一的 API URL 和 API Key，并维护多个可用模型。右侧输入区可在
“权限”和“Canvas”之间快速切换当前模型；新的普通回复、Canvas 规划和 Agent Run 都会使用该
选择。API Key 只保存在本机 SQLite 中，后端不会向页面回传明文；未保存 Web 配置时仍使用根目录
`.env` 作为后备来源。
...
对话中生成的计划和执行任务会保存为带版本号的任务草案快照；消息与 Run 引用同一草案版本，
为后续 Canvas 提案、用户编辑确认和按确认版本执行提供一致的数据基础。
Canvas 编辑草稿按 Session 分开保存在本地；新会话使用通用任务拆解示例，也可一键清空当前画布。
当 Agent 判断用户正在请求规划时，会生成独立的只读 Canvas 摘要：画布只保留目标、少量高层
任务模块和汇总后的关键约束，完整验收细节继续保存在任务草案中。用户可先预览，再采用到当前
Canvas 中继续编辑，最后通过“确认并执行”生成新的确认版本并启动 Run；未在摘要中改动的验收
细节会继续保留，提案也不会直接覆盖原画布。
